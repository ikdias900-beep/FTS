...
| `ASM-FFF-0001` | 1 | total-order morphism transcription | Does the R-status implementation count only order-preserving maps, also order-reversing maps, and/or constant maps under the exact source definitions? | An intuitive implementation could diverge from Appendix A.1/A.2 and double-count edge cases. | `SRC-FFF-2020` | `CLM-FFF-ORD-001` | source-exact variants to be transcribed | HIGH | OPEN | — | — | Two independent agents transcribe definitions and enumerate hand-worked small cases before code. | This is a formalization risk, not permission to choose a convenient definition. |
