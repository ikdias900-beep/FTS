...
|---|---:|---|---|---|---|---|---|---|---|---|---|---|---|
| `ASM-FBT-0001` | 2/4 | tie handling | How should a general Truth strategy act when multiple world states share the maximum posterior probability? | The numerical example has a unique MAP result, but the reusable engine must not silently choose a tie rule. | `SRC-FBT-2021` | future general FBT claims | return a set; deterministic lexical rule; randomize uniformly; preserve full posterior | HIGH | APPROVED | Human PI | 2026-07-01 | Stage 4 tests must cover unique MAP, same-decision MAP ties, and `map_tie_policy_sensitive` ties; primary results must not use lexical or random tie-breaks. | Approved via `RDR-0004`: represent MAP ties as full MAP sets and classify tie-sensitive cells separately. Does not block exact Stage 2 calculations if no tie occurs. |
| `ASM-FBT-0002` | 2/4 | zero-probability observation | What should the API return when an observation has zero marginal probability? | Bayes posterior is undefined; silent normalization would fabricate information. | `SRC-FBT-2021` | future general FBT claims | explicit exception; undefined result object; exclude by domain contract | MEDIUM | APPROVED | Human PI | 2026-07-01 | Stage 4 tests must cover `P(x) = 0`, `zero_marginal_undefined`, and `blocked_zero_marginal`; exact primary paths must not smooth with epsilon. | Approved via `RDR-0004`: no smoothing; zero-marginal observations are undefined and block dependent comparisons. Stage 2 source example has nonzero marginals. |
| `ASM-FBT-0003` | 4 | finite atlas grid measure | What finite rational grid and measure semantics may be used for the Stage 4 atlas? | A project grid frequency is not the source theorem probability; changing the grid can change headline aggregate patterns. | `SRC-FBT-2021` | `CLM-FBT-ATLAS-001` | fixed uniform rational grid; weighted grid; stratified grid with per-family reporting; no aggregate probability claim | HIGH | APPROVED | Human PI | 2026-07-01 | Stage 4 manifests and reports must pin `grid_version`, use `grid_frequency` labels, preserve denominator accounting for blocked/tie-sensitive cells, and forbid source-theorem-probability wording for grid counts. | Approved via `RDR-0004`: use frozen finite grid versions and report `grid_frequency`, never source theorem probability. Introduced by `TASK-004-FBT-ATLAS-SPEC`; does not block source transcription. |
| `ASM-FBT-0004` | 4 | truth strategy family | Which truth-strategy variants are part of the primary Stage 4 comparison, and how are source MAP Truth and extension baselines separated? | Posterior sampling, posterior mean, and full-posterior baselines can change the comparison and must not be mislabeled as the source Truth strategy. | `SRC-FBT-2021` | `CLM-FBT-ATLAS-001` | source MAP only; source MAP plus separately reported extension baselines; full posterior as primary baseline | HIGH | APPROVED | Human PI | 2026-07-01 | Stage 4 tests and reports must keep primary `truth_map` vs `fitness_only_expected` separate from extension baselines; no aggregate may merge all truth-like strategies under one source label. | Approved via `RDR-0004`: primary comparison is `truth_map` vs `fitness_only_expected`; extension baselines are separate. Posterior mean requires an approved metric or vector structure on `W`. |
